# -*- coding: utf-8 -*-
"""
Created on Tue Nov 28 22:40:56 2017
...
def model_inference_critisism(model,Bayesian,seq_array1,seq_array2,
                              label_array1,label_array2,sequence_length,N,D,
                              H=100,H1=100,H2=50):
    if Bayesian == False:
        model = Sequential()
        if model == 'Fully Connected Layer':
            model.add(Dense(H,
                     input_shape=[D],activation='tanh'))
        elif model == 'Simple RNN':
            model.add(SimpleRNN(
        ...
        elif model == 'Two Layer Simple RNN':
            model.add(SimpleRNN(
                     input_shape=(sequence_length, D),
                     units=H1,return_sequences=True))
            model.add(SimpleRNN(
                     units=H2))
        elif model == 'Two Layer LSTM':
            model.add(LSTM(
                     input_shape=(sequence_length, D),
                     units=H1,return_sequences=True))
            model.add(LSTM(
                     units=H2))
        elif model == 'Two Layer GRU':
            model.add(GRU(
                    input_shape=(sequence_length, D),
                    units=H1,return_sequences=True))
            model.add(GRU(
        ...
        #Histogram
        sns.distplot(y_pred)
        plt.title(f'Histogram of RUL, Frequentist {model}')
        plt.xlabel('RUL');
        #RMSE
      ...
      y_pred[less_50]))}')
        ...
        plt.title(f'Prediction of RUL, Frequentist {model}')
        plt.xlabel('RUL');
        ...
        plt.title(f'Histogram of RUL, Bayesian {model}')
        plt.xlabel('RUL');
        #RMSE
      ...
      y_pred[less_50])) for y_pred in y_preds])}')
        ...
        plt.title(f'Prediction of RUL, Bayesian {model}')
        plt.xlabel('RUL');
        ...
        plt.title(f'Distribution of Prediction of RUL, Bayesian {model}')
        ...
        raise Exception('Please specify a boolean for use Bayesian inference or not!')